...
module.exports.create = (privateKey, applicationId, baseApiUrl, timeout) => {
  ...
  return app.connect(timeout).then(() => {
    return app;
  });
};
...
class GitHubApplication {
  ...
  getRepositoryInstallation(owner, repo) {
    return this.client.rest.apps
      ...
      .catch((err) => {
        throw new Error(
          `Failed to resolve installation of application on repository ${owner}/${repo}; ${err.message}`,
        );
      })
      .then((resp) => {
        if (resp.status === 200) {
  ...
  getOrganizationInstallation(org) {
    return this.client.rest.apps
      ...
      .catch((err) => {
        throw new Error(
          `Failed to resolve installation of application on organization ${org}; ${err.message}`,
        );
      })
      .then((resp) => {
        if (resp.status === 200) {
  ...
  getInstallationAccessToken(installationId, permissions) {
    ...
    return this.client
      .request(`POST /app/installations/${installationId}/access_tokens`, {
        mediaType: {
          previews: ['machine-man'],
        },
        ...additional,
